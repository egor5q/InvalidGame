...
def donate(m):
   bot.send_message(m.chat.id, 'Донат - покупка игровых ресурсов за реальные деньги.\n'+
                    ...
                    'При покупке от 500р начисляется бонус - дополнительные 1200⚛.', parse_mode='markdown')
...
def inline(call): 
  ...
  if call.data=='hp':
       ...
       kb.add(types.InlineKeyboardButton(text='Назад', callback_data='back'))
       medit('Этот скилл даёт боту возможность восстанавливать себе 1 хп каждые 6 ходов. Хотите приобрести?',call.message.chat.id, call.message.message_id, reply_markup=kb)
